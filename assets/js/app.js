export default class AppController{
    constructor(){
        ...
        this.baseURL = `https://api.benitrade.net`;
        ...
        */
        setTimeout(async ()=>{
            this.fetchJSON("pairs",`${this.baseURL}/pairs?ERC20=true`);
            this.fetchJSON("limits",`${this.baseURL}/limits`);
        },100);
    ...
    async getTrade(inToken, outToken, amtD, amtW, userAddr, refundAddr){
        ...
        let url = new URL(`${this.baseURL}/trade/${inToken}/${outToken}/${userAddr}`);
        if(refundAddr){
    ...
    async getStatus(id){
        ...
        let url = new URL(`${this.baseURL}/status/${id}`);
        console.debug("fetching: ",url);
    ...
    async getQuote(inToken,outToken, amtD, amtW){
        ...
        let url = new URL(`${this.baseURL}/quote/${inToken}/${outToken}`);
        if(amtW > 0){
            url.searchParams.append("amtw",amtW);
        }else{
            if(amtD > 0){
